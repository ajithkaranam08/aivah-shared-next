...
export const connectLiveKitAudio = (mediaStream: MediaStream): void => {
  ...
  try {
    ...
    console.error(
      "WawaLipsyncManager: Failed to connect LiveKit audio:",
      error
    );
  }
};
...
export const disconnectLiveKitAudio = (): void => {
  if (currentLiveKitStream || liveKitSourceNode) {
    try {
      ...
      console.warn(
        "WawaLipsyncManager: Error disconnecting LiveKit audio:",
        error
      );
    }
  }
...
const analyzeLiveKitAudio = (): {
  ...
  const bands = [
    ...
    dataArray.slice(lowEnd, lowMidEnd).reduce((a, b) => a + b, 0) /
      (lowMidEnd - lowEnd),
    dataArray.slice(lowMidEnd, midEnd).reduce((a, b) => a + b, 0) /
      (midEnd - lowMidEnd),
    dataArray.slice(midEnd, highMidEnd).reduce((a, b) => a + b, 0) /
      (highMidEnd - midEnd),
    dataArray
      .slice(highMidEnd, Math.min(highEnd, bufferLength))
      .reduce((a, b) => a + b, 0) /
  ...
  const normalized =
    totalEnergy > 0 ? bands.map((b) => b / totalEnergy) : [0, 0, 0, 0, 0];

  let viseme = "viseme_sil";
  if (smoothedVolume < SILENCE_THRESHOLD) viseme = "viseme_sil";
  else if (smoothedVolume > VOLUME_THRESHOLD) {
    if (normalized[4] > 0.3 && normalized[3] > 0.25)
      viseme = normalized[4] > normalized[3] ? "viseme_SS" : "viseme_CH";
    else if (normalized[0] > 0.4 && normalized[2] < 0.2) viseme = "viseme_U";
    else if (normalized[2] > 0.4 && normalized[0] < 0.3)
      viseme = normalized[3] > normalized[1] ? "viseme_I" : "viseme_E";
  ...
  if (previousVisemes.length > 0) {
    ...
    const minDuration =
      lastViseme === "viseme_sil"
        ? MIN_VISEME_DURATION * 0.5
        : MIN_VISEME_DURATION;
    if (
      currentTime - lastVisemeTime < minDuration &&
      lastViseme !== "viseme_sil"
    )
      viseme = lastViseme;
  }
    ...
    if (currentLiveKitStream && liveKitAnalyser) {
      ...
      if (customAnalysisResult) {
        setLipsyncData({
          ...
          lastActiveTime:
            customAnalysisResult.volume > 0.001 ? Date.now() : undefined,
      ...
      setLipsyncData({
        ...
        lastActiveTime:
          getLipsyncManager()?.viseme !== "viseme_sil" ? Date.now() : undefined,
        intensity: volume,
      });
    }
