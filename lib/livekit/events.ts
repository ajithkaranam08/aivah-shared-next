...
import { Room, RoomEvent, RemoteParticipant, RemoteTrack, Track, DisconnectReason, TranscriptionSegment } from "livekit-client";
...
export type RoomEventOptions = {
  ...
  onTranscription?: (segments: TranscriptionSegment[] | TranscriptionSegment) => void;
};

export function attachRoomEventHandlers(room: Room, opts: RoomEventOptions = {}) {
  const { audioContainer, onConnected, onDisconnected, onAudioPlayingChange } = opts;
  // Connected
    ...
    if (track.kind === Track.Kind.Audio && audioContainer) {
        ...
        try {
            ...
            if (audioTrack.mediaStream) {
              ...
              console.log('✅ LiveKit audio connected to both orb visualization and lipsync');
            }
          } catch (error) {
            console.warn('Failed to connect LiveKit audio to visualization:', error);
          }
  ...
  room.on(RoomEvent.TranscriptionReceived, (transcription) => {
    ...
    opts.onTranscription?.(transcription as unknown as TranscriptionSegment[] | TranscriptionSegment);
  });
