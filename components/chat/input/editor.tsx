...
const EditorInput = ({ onExpand }: { onExpand: (val: ChatInputExpandTypes) => void }) => {

    const editorRef = useRef<HTMLDivElement>(null);
    const form = useFormContext<ChatFormType>();

    const handleInput = (e: React.FormEvent<HTMLDivElement>, onChange: (v: string) => void) => {
        ...
        const placeholder = div.querySelector('[data-placeholder]') as HTMLParagraphElement;
        if (placeholder) {
        ...
        if (e.key === 'Enter') {
            ...
            form.handleSubmit((data) => {
                ...
                if (editorRef.current)
                    editorRef.current.innerHTML = '<p data-placeholder="Ask anything" class="place-holder"></p>';
            })();
        }
    };
