...
const AVATAR_PLACEMENT: Record<string, { position: [number, number, number], rotation: [number, number, number] }> = {
    face: {
        position: [0, 0, 0],
        rotation: [0, 0, 0],
    },
...
const Experience = ({
    ...
    const avatarPlacement = scene === "empty" ? AVATAR_PLACEMENT["face"] : AVATAR_PLACEMENT[scene]
...
type CameraVariants = "face" | "body" | "zen" | "dashboard" | "webresults" | "videowall" | "orbe" | "presentation" | "presentation2";

const CAMERA_SETTINGS: Record<CameraVariants, { lookAt: number[]; fov: number }> = {
    face: {
...
interface CameraHandlerProps {
    focus?: "face" | "body";
    scene?: "videowall" | "empty" | "zen" | "webresults" | "presentation" | "presentation2";
    companionType?: string;
}

const CameraHandler = ({ focus = "face", scene = "empty", companionType }: CameraHandlerProps) => {
    const controls = useRef<CameraControls | null>(null);

    const resetCameraPosition = useCallback(() => {
        if (controls.current) {
            const camera = controls.current.camera;
            const camSettings = scene === "empty" ? CAMERA_SETTINGS["body"] : CAMERA_SETTINGS[scene];
            ...
            controls.current.setLookAt(lookAt[0], lookAt[1], lookAt[2], lookAt[3], lookAt[4], lookAt[5], true);
            //@ts-ignore
            camera.fov = fov;
