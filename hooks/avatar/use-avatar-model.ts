...
export function useAvatarModel(modelUrl: string) {
    ...
    const { nodes: morphNodes } = useGLTF('/models/BaseModelOptimized.glb') as GLTFWithNodes;
    ...
    useEffect(() => {
        ...
        if (dst && src) {
            ...
            if ((!dst.morphTargetInfluences || dst.morphTargetInfluences.length === 0) && src.morphTargetInfluences) {
